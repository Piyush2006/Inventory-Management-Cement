    ...
    <div className="space-y-4">
      ...
      {canCreate && (
        <div className="space-y-3">
          ...
          <Modal open={showCreate} onClose={() => setShowCreate(false)} title="New Dispatch">
            <form
            ...
            >
              <label className="text-xs text-muted">
                Material
                <select name="materialId" value={materialId} onChange={(e) => setMaterialId(e.target.value)} required className="mt-1 block w-full rounded-md border border-border bg-surface-raised px-2.5 py-1.5 text-sm text-foreground outline-none focus:border-accent">
                  {materials.map((m) => (
              ...
              <label className="text-xs text-muted">
                Source Location
                <select name="sourceLocationId" value={sourceLocationId} onChange={(e) => setSourceLocationId(e.target.value)} required className="mt-1 block w-full rounded-md border border-border bg-surface-raised px-2.5 py-1.5 text-sm text-foreground outline-none focus:border-accent">
              ...
              <label className="text-xs text-muted sm:col-span-3">
                ...
                <input name="customerDestination" required className="mt-1 block w-full rounded-md border border-border bg-surface-raised px-2.5 py-1.5 text-sm text-foreground outline-none focus:border-accent" />
              </label>
              <label className="text-xs text-muted">
                Batch / Lot (optional)
              ...
              <label className="text-xs text-muted">
                ...
                <input name="weighmentReference" className="mt-1 block w-full rounded-md border border-border bg-surface-raised px-2.5 py-1.5 text-sm text-foreground outline-none focus:border-accent" />
              </label>
              <label className="text-xs text-muted">
                Notes (optional)
                <input name="notes" className="mt-1 block w-full rounded-md border border-border bg-surface-raised px-2.5 py-1.5 text-sm text-foreground outline-none focus:border-accent" />
              </label>
              ...
              <div className="flex gap-2 sm:col-span-3">
                ...
                <button type="button" onClick={() => setShowCreate(false)} className="btn btn-secondary btn-md">
                  Cancel
                </button>
              </div>
            </form>

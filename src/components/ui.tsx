...
export function Panel({ title, action, children, className = "" }: { title?: ReactNode; action?: ReactNode; children: ReactNode; className?: string }) {
  return (
    <section className={`shadow-panel rounded-lg border border-border bg-surface transition-shadow ${className}`}>
      {title && (
...
const KPI_TONE_BAR: Record<string, string> = {
...
export function KpiTile({
  ...
  return (
    <div className="group relative shadow-panel overflow-hidden rounded-lg border border-border bg-surface px-4 py-3.5 transition-shadow hover:shadow-[var(--shadow-md)]">
      <span className={`absolute inset-y-0 left-0 w-0.5 ${KPI_TONE_BAR[tone]}`} />
